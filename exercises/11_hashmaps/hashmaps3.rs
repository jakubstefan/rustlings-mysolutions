...
fn build_scores_table(results: &str) -> HashMap<&str, TeamScores> {
    ...
    for line in results.lines() {
        ...
        let goals = scores.entry(team_1_name).or_insert(
            TeamScores{goals_scored: 0, goals_conceded: 0}); // starts the count at 0
        goals.goals_scored += team_1_score;
        goals.goals_conceded += team_2_score;

        let goals = scores.entry(team_2_name).or_insert(
            TeamScores{goals_scored: 0, goals_conceded: 0}); // starts the count at 0
